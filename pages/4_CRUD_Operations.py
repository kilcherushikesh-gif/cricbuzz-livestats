...
teams_df = run_query("SELECT team_id, team_name FROM teams")
    ...
    with st.form("create_form"):
        ...
        if submitted:
            if not full_name:
                ...
                run_action(
                    """INSERT INTO players (full_name, team_id, playing_role, batting_style, bowling_style)
                       VALUES (?, ?, ?, ?, ?)""",
                    (full_name, team_options.get(team_name), role, batting_style, bowling_style or None),
                )
...
with tab_update:
    ...
    df = run_query("SELECT player_id, full_name FROM players ORDER BY player_id")
    if df.empty:
        ...
        with st.form("update_form"):
            ...
            new_role = st.text_input("Playing role", current["playing_role"] or "")
            ...
            if st.form_submit_button("Save changes"):
                run_action(
                    """UPDATE players SET full_name=?, playing_role=?, batting_style=?, bowling_style=?
                       WHERE player_id=?""",
                    (new_name, new_role, new_batting, new_bowling, pid),
                )
                st.success("Updated.")
...
with tab_delete:
    ...
    if df.empty:
        ...
        options = dict(zip(df["full_name"], df["player_id"]))
        chosen = st.selectbox("Player to delete", list(options.keys()), key="delete_select")
        if st.button("🗑️ Delete this player", type="primary"):
            run_action("DELETE FROM players WHERE player_id = ?", (options[chosen],))
            st.success(f"Deleted {chosen}. Refresh the page to see the updated list.")
